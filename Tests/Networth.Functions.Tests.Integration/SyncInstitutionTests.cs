    ...
    : IntegrationTestBase(mockoonTestFixture, testOutput)
{
    ...
    public async Task SyncInstitution_EnqueuesSyncForLinkedAccounts()
    {
        ...
        // Act
        var response = await httpClient.PostAsync($"/api/institutions/{institutionId}/sync", null);

        // Assert
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var result = await response.Content.ReadFromJsonAsync<SyncInstitutionResponse>();
        Assert.NotNull(result);
        Assert.Equal(2, result.AccountsEnqueued);
    }
    ...
    public async Task SyncInstitution_EnqueuesSyncForSingleAccount()
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<SyncInstitutionResponse>();
        Assert.NotNull(result);
        Assert.Equal(1, result.AccountsEnqueued);
    }
    ...
    public async Task SyncInstitution_ReturnsZeroForNoAccounts()
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<SyncInstitutionResponse>();
        Assert.NotNull(result);
        Assert.Equal(0, result.AccountsEnqueued);
    }

    private record SyncInstitutionResponse(int AccountsEnqueued);
}
